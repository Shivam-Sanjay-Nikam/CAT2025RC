...
import { ArrowLeft, CheckCircle2, XCircle, RotateCcw, Crown, Scroll } from 'lucide-react';
...
const EssayPage: React.FC<EssayPageProps> = ({ essayId, onBack, onShowResults }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const foundEssay = data.essays.find(e => e.id === essayId);
  ...
  if (!essay) {
    return <div className="flex items-center justify-center min-h-screen">
      <div className="text-xl text-slate-600 font-light">Preparing your assessment...</div>
    </div>;
  }
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 p-8">
          <div className="mb-8">
            <div className="flex items-center mb-4">
              <Scroll className="text-amber-600 mr-3" size={28} />
              <h1 className="text-3xl font-serif text-slate-900 tracking-wide">
              {essay.title}
            </h1>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 p-8">
            <div className="flex items-center mb-8">
              <Crown className="text-amber-600 mr-3" size={24} />
              <h2 className="text-2xl font-serif text-slate-900">
                Assessment Questions
              </h2>
              <span className="ml-auto text-slate-500 font-light">({essayQuestions.length})</span>
            </div>
            <div className="w-12 h-1 bg-gradient-to-r from-amber-400 to-amber-600 mb-8"></div>
              ...
              {essayQuestions.map((question, index) => (
                <div key={question.id} className="border-b border-slate-100 pb-8 last:border-b-0">
                  <div className="flex items-start mb-6">
                    <div className="w-8 h-8 bg-gradient-to-br from-slate-700 to-slate-900 rounded-full flex items-center justify-center text-white font-serif text-sm mr-4 mt-1">
                      {index + 1}
                    </div>
                    <h3 className="text-lg font-serif text-slate-900 leading-relaxed flex-1">
                      {question.question}
                  </h3>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3 w-full">
                            ...
                            <span className="text-slate-800 font-light">{option.text}</span>
            ...
            {!showResults ? (
              <button
                ...
                className={`flex-1 py-4 px-8 rounded-xl font-medium transition-all duration-300 shadow-lg ${
                  allAnswered
                    ? 'bg-gradient-to-r from-slate-800 to-slate-900 text-white hover:from-slate-700 hover:to-slate-800 hover:shadow-xl'
              ...
              <button
                onClick={resetTest}
                className="flex-1 py-4 px-8 rounded-xl font-medium bg-gradient-to-r from-amber-600 to-amber-700 text-white hover:from-amber-500 hover:to-amber-600 transition-all duration-300 flex items-center justify-center gap-3 shadow-lg hover:shadow-xl"
              >
                <RotateCcw size={18} />
                Retake Assessment
              </button>
